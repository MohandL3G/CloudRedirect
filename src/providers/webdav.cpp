...
static std::string Base64Encode(const std::string& in) {
    DWORD outLen = 0;
    CryptBinaryToStringA((const BYTE*)in.data(), (DWORD)in.size(),
...
bool WebDavProvider::ParseUrl(const std::string& url, std::string& host,
                               int& port, std::string& basePath, bool& isHttps) {
...
bool WebDavProvider::Upload(const std::string& path, const uint8_t* data, size_t len) {
    ...
    auto r = Request("PUT", GetFullUrl(path),
...
static int64_t ParseHttpDate(const std::string& date) {
    struct tm tm = {};
    char monthStr[4];
    if (sscanf(date.c_str(), "%*s %d %3s %d %d:%d:%d",
...
bool WebDavProvider::ListChecked(const std::string& prefix,
    ...
    size_t pos = 0;
    while ((pos = r.body.find("<d:response", pos)) != std::string::npos ||
           (pos = r.body.find("<D:response", pos)) != std::string::npos) {
        ...
        if (hostPos != std::string::npos) {
            size_t slashPos = decodedHref.find('/', hostPos + m_host.length());
            if (slashPos != std::string::npos)
                decodedHref = decodedHref.substr(slashPos);
        }

        std::string normPath = path;
        if (normPath.length() > 0 && normPath[0] != '/') normPath = "/" + normPath;
        if (decodedHref == normPath || decodedHref == normPath.substr(0, normPath.length() - 1))
            continue;
